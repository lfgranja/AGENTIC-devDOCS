...
class IssueCreator:
    ...
    def _parse_issue_section(self, section: str) -> Optional[IssueInfo]:
        ...
        """
        # Extract title
        title_match = re.search(r'^\s*[*]{2}Title[*]{2}:\s*(.+?)$', section, re.MULTILINE)
        if not title_match:
        ...
        # Extract description
        desc_match = re.search(r'^\s*[*]{2}Description[*]{2}:\s*(.+?)$', section, re.MULTILINE | re.DOTALL)
        if not desc_match:
        ...
        # Extract labels
        labels_match = re.search(r'^\s*[*]{2}Labels[*]{2}:\s*(.+?)$', section, re.MULTILINE)
        labels = []
        if labels_match:
        ...
        # Extract milestone
        milestone_match = re.search(r'^\s*[*]{2}Milestone[*]{2}:\s*(.+?)$', section, re.MULTILINE)
        milestone = milestone_match.group(1).strip() if milestone_match else None
        
        # Extract priority
        priority_match = re.search(r'^\s*[*]{2}Priority[*]{2}:\s*(.+?)$', section, re.MULTILINE)
        priority = priority_match.group(1).strip() if priority_match else None
        
        # Extract complexity
        complexity_match = re.search(r'^\s*[*]{2}Complexity[*]{2}:\s*(.+?)$', section, re.MULTILINE)
        complexity = complexity_match.group(1).strip() if complexity_match else None
        
        # Extract context
        context_match = re.search(r'^\s*[*]{2}Context[*]{2}:\s*(.+?)$', section, re.MULTILINE | re.DOTALL)
...
def main():
    ...
    repo_name = args.repo or config_manager.get('repo_name')
    pr_number = args.pr or config_manager.get('pr_number')
    issues_file_path = args.issues_file or config_manager.get('issues_file_path')
    
    # Validate required parameters
    if not github_token:
        parser.error("GitHub token is required. Provide with --token or in config file/environment variable.")
    if not repo_name:
        parser.error("Repository name is required. Provide with --repo or in config file.")
    if not pr_number:
        parser.error("PR number is required. Provide with --pr or in config file.")
    if not issues_file_path:
        parser.error("Issues file path is required. Provide with --issues-file or in config file.")
